...
- **P0 범위 (MVP)**: 소셜 로그인(카카오, 구글 OAuth2 2종) + 세션 유지, 위치 권한/현재 위치 조회, 현재 위치 중심 지도, 주변 스팟 조회·마커 표시(지도 이동 시 재조회), 스팟 상세(사진·거리·주소), 스팟 등록(위치 지정 + 사진 1장)
...
| 인증 | OAuth2 소셜 로그인(카카오, 구글 2종) + 자체 JWT 발급 |
...
### Phase 1 — 인증 & 기본 지도 (2주)
- 카카오/구글 소셜 로그인 연동 (FE/BE), provider별 OAuth2 어댑터 설계, JWT 발급·검증
...
**Phase 1 — 인증 & 지도**
- [ ] 카카오/구글 로그인 버튼 각각을 통해 로그인 완료 시 JWT가 발급되고, 앱을 종료 후 재실행해도 재로그인 없이 지도 화면으로 진입한다
...
- **Phase 1**: 실기기에서 카카오/구글 각각 로그인 → 앱 강제 종료 → 재실행 → 자동 로그인(세션 유지) 확인

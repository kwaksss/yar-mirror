...
- **P0 범위 (MVP)**: 소셜 로그인(1개 이상) + 세션 유지, 위치 권한/현재 위치 조회, 현재 위치 중심 지도, 주변 스팟 조회·마커 표시(지도 이동 시 재조회), 스팟 상세(사진·거리·주소), 스팟 등록(위치 지정 + 사진 1장)
...
| 인증 | OAuth2 소셜 로그인(카카오 우선) + 자체 JWT 발급 |
| 스토리지 | 오브젝트 스토리지 (S3 or 호환, 스팟 사진 업로드) |
...
```
[RN App] --REST(JWT)--> [Spring Boot API] --> [PostgreSQL+PostGIS]
                                          \--> [Object Storage(사진)]
                                          \--> [Kakao OAuth]
```
...
- **Spot**: id, uploaderId, latitude, longitude, address, name, description, photoUrl, createdAt
...
| Method | Endpoint | 설명 |
|---|---|---|
| POST | `/auth/login/{provider}` | 소셜 로그인 → JWT 발급 |
| GET | `/auth/me` | 내 프로필 조회 |
| GET | `/spots?lat=&lng=&radius=` | 주변 스팟 조회 (지도 영역 기준) |
| GET | `/spots/{id}` | 스팟 상세 조회 |
| POST | `/spots` | 스팟 등록 (사진 multipart 포함) |
...
- 오브젝트 스토리지 버킷/자격증명 세팅

### Phase 1 — 인증 & 기본 지도 (2주)
- 카카오 소셜 로그인 연동 (FE/BE), JWT 발급·검증
- 로그인 세션 유지 (RN SecureStore/AsyncStorage + refresh 전략)
- 위치 권한 요청 및 현재 위치 조회
- 현재 위치 중심 카카오맵 표시

### Phase 2 — 스팟 조회 (2주)
- 반경/영역 기반 스팟 조회 API (PostGIS `ST_DWithin` 쿼리)
...
- 위치 지정 (현재 위치 or 지도 탭)
- 스팟 등록 폼 (이름 필수, 설명 선택)
- 등록 후 지도 즉시 반영 (로컬 상태 optimistic update)
...
**Phase 1 — 인증 & 지도**
- [ ] 카카오 로그인 완료 시 JWT가 발급되고, 앱을 종료 후 재실행해도 재로그인 없이 지도 화면으로 진입한다
- [ ] 위치 권한을 거부하면 권한 안내 UI가 표시되고, 허용 시 5초 이내 현재 위경도가 조회된다
- [ ] 지도 최초 진입 시 현재 위치가 중심점으로 표시된다

**Phase 2 — 스팟 조회**
- [ ] 현재 위치 기준 반경 내 등록된 스팟이 모두 마커로 표시된다 (등록된 개수와 마커 개수 일치)
...
| 이미지 업로드 관리 미흡 | 저장 비용/부적절한 콘텐츠 증가 | 업로드 시 용량·포맷 제한, 추후 신고 기능(P1)로 대응 |
| 소셜 로그인 정책 변경 (카카오 등) | 인증 플로우 중단 | OAuth 연동을 인증 어댑터 계층으로 분리해 대체 provider 추가 용이하게 설계 |

## 9. Verification Steps

- **Phase 0**: PoC 빌드에서 카카오맵이 RN 화면에 실제 렌더링되는지 실기기/시뮬레이터로 확인
- **Phase 1**: 실기기에서 로그인 → 앱 강제 종료 → 재실행 → 자동 로그인(세션 유지) 확인
- **Phase 2**: `curl`/Postman으로 `/spots?lat=&lng=&radius=` 응답 검증 + `EXPLAIN ANALYZE`로 GiST 인덱스 사용 확인
- **Phase 3**: 실기기에서 사진 업로드 후 오브젝트 스토리지에 파일 존재 확인 + DB row 저장 확인
- **Phase 4**: 체크리스트 기반 P0 전체 E2E 시나리오 수동 QA (로그인 → 탐색 → 상세 → 등록 → 재조회), 오류 상황(권한 거부, 네트워크 끊김) 별도 확인

## 10. 다음 단계
- Phase 0 착수: 프로젝트 초기 세팅 + 카카오 API 키 발급
- 카카오맵 RN 연동 PoC 진행
